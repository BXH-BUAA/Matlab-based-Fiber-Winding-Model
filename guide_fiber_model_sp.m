...
L_fiber = 30e9;%光纤长度
R_fiber = 280;%g光纤理想直径
E_fiber = 0*R_fiber*0.10;%光纤挤压最大允许挤压百分比
B_fiber = R_fiber*0.10;%光纤环底层最小间距
conf_int = R_fiber*10/1000;%光纤直径最大偏差
min_fiber = R_fiber*0.12;%光纤之间间隙初步控制
min_edge = 0;%底层光纤距光纤环骨架底边距离

%采样参数
N_samp = 4;%每个圆环中的采样个数，模型中提供4、5、6三种采样个数
...
%返回次数过多时是否重新选择底层匝数,{ 0 }表示立即终止计算,{ 1 }表示重新选择匝数进行计算，{ 2 }表示继续进行计算
restart_model = 2;
...
for N_bot = N_bot_max_temp - round(N_bot_max_temp/75):round(N_bot_max_temp/100):N_bot_max_temp + round(N_bot_max_temp/75)
    ...
    if re_read == 1
        disp('读取上次运行的直径矩阵');
        load('last_diam.mat'); %再次读入上次运行的直径矩阵
    else
        disp('自行生成直径矩阵');
    ...
    ints_temp_save = [];
    ints_neg_save = [];%记录层级推算中的负间隙
    while  sat == 0
        if length(error_level) > error_num
            if restart_model == 1
                disp('返回次数过多！重新选择底层匝数！')
                break;
        ...
        if jump_jump == 0
            ...
            ints_mat_bot_rem = S_base - max(sum(diam_mat_bot, 2)) - (N_bot_temp - 1)*B_fiber -min_edge*2;%减10um是考虑到冗余
            ...
            ints_mat_bot_temp = ints_mat_bot;
                ...
                if ints_mat_bot_save(:,ints_layer) < -1*E_fiber||ints_mat_bot_save(:,ints_layer) > (sqrt(3)-1)/2*R_fiber
                    error('底层级间隙出现严重错误，请重新选择底层级匝数或光纤采样点位置！');
                end
